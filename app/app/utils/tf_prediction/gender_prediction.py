#!/usr/bin/env python
# # -*- coding: utf-8 -*-
""" === gender_prediction.py: script for exporting gender prediction model into pb format === """
...
tf.app.flags.DEFINE_string('model_dir_gender', 'models/checkpoints/21936',
                           'Gender model directory (where training data lives)')
...
tf.app.flags.DEFINE_string('output_dir', './gender_est-export',
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session(config=config) as sess:
        ...
        label_list_gender = GENDER_LIST
        nlabelsg = len(label_list_gender)

        model_fn = select_model(FLAGS.model_type)
            
        with tf.device(FLAGS.device_id):
            
            logits = model_fn(nlabelsg, images, 1, False)
            ...
            checkpoint_path = '%s' % (FLAGS.model_dir_gender)
            ...
            print("Successfully exported GEN-EST model version '{}' into '{}'".format(FLAGS.model_version, FLAGS.output_dir))
